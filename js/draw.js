...
Draw.roller = function (x, y, angle, color) {
  var ctx = Draw.ctx;
  var r = CONFIG.PLAYER_RADIUS;
  var centerX = x + CONFIG.PLAYER_SIZE / 2;
  var centerY = y + CONFIG.PLAYER_SIZE / 2;
  ctx.fillStyle = color;
  ctx.beginPath();
  ctx.arc(centerX, centerY, r, 0, Math.PI * 2);
  ctx.fill();
  var dotX = centerX + Math.cos(angle) * r * CONFIG.DOT_DISTANCE;
  var dotY = centerY + Math.sin(angle) * r * CONFIG.DOT_DISTANCE;
  ctx.fillStyle = "#000000";
  ctx.beginPath();
